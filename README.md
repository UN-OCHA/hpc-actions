...
- Pushes to `env/prod` and `env/<stage|staging>`:
  - Check if there is an existing tag for the current version in `package.json`
    - TODO: _(we'll need to allow for specifying the version in other ways for the
      non-node repos)._
  ...
  - Check if there is an existing docker image tag for the given version.
    - If there is not
      ...
      - Fetch the git tag to check the git tree hash has not been changed
        _(this will only happen with rapid concurrent pushes)_
...
_(for full details of what you can put in your configuration file,
please see the [`config.ts` module](https://github.com/UN-OCHA/hpc-actions/blob/develop/action/src/config.ts))_
